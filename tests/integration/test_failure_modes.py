...
Dependencies are broken by pointing the store at an unreachable address, so the code under test
takes its real error path — a mock that raises would prove only that the ``except`` clause is
reachable.
"""
...
async def test_rate_limiting_fails_open_when_redis_is_unreachable(
    ...
    Failing closed would convert a cache outage into a total authentication outage for every
    user — a self-inflicted denial of service far more likely than the brute-force campaign the
    limiter defends against. The controls that actually stop credential attacks do not live in
...
async def test_rate_limiting_can_be_configured_to_fail_closed(container: Container) -> None:
    """A deployment with a different threat model flips one setting and gets the opposite
    behaviour."""
...
async def test_the_access_token_denylist_fails_open(container: Container) -> None:
    ...
    Failing closed would break every API call in the deployment during a Redis outage. Failing
    open only affects tokens explicitly revoked inside the current access-token lifetime —
    minutes — and the refresh token underneath was revoked in Postgres, so the session cannot be
    extended either way.
    """
...
async def test_login_fails_cleanly_when_redis_is_unreachable(
    ...
    §21 distinguishes an availability dependency from a security one: without Redis there is
    nowhere to put a session, so login legitimately cannot complete. What must not happen is a
    500 with a stack trace, or worse, a login that appears to succeed without a session.
    """
    ...
    try:
        ...
        container.redis._client = original
...
async def test_the_liveness_probe_stays_green_when_datastores_are_down(
    ...
    If the ALB's health check touched Postgres, a brief RDS failover would fail every task's
    check at once and ECS would replace the whole fleet — turning a recoverable blip into an
    outage. Liveness answers only "can this process serve HTTP".
    """
    original = container.redis._client
    container.redis._client = _broken_redis()
    try:
...
async def test_token_verification_survives_a_redis_outage(
    ...
    This is the concrete payoff of signing access tokens instead of storing them: /userinfo needs
    only the signing key's public half, which is cached from Postgres.
    """
...
async def test_discovery_and_jwks_survive_a_redis_outage(
    ...
    """A relying party bootstrapping during a cache outage must still be able to discover the
    IdP."""
    original = container.redis._client
    container.redis._client = _broken_redis()
    try:
...
async def test_an_unexpected_error_is_reported_without_internal_detail(
    ...
    The exception is logged in full, with a request ID to correlate it, but the caller receives
    OAuth's own opaque ``server_error``.
    """
    original = container.database._engine
    container.database._engine = None
    try:
        ...
        container.database._engine = original
...
async def test_a_signing_key_whose_private_material_is_missing_fails_loudly(
    ...
    against another environment's secret store. It must be an obvious error, not a silent
    inability to sign."""
