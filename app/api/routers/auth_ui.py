...
async def mfa_submit(
    ...
    try:
        ...
        # The pending state was destroyed, so there is nothing to retry against: send the user
        # back to the password step rather than to a form that can no longer succeed.
        flow = await csrf.issue_token_for_flow(request, container)
        response = render(
            request,
            "login.html",
